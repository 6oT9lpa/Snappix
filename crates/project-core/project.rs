...
use crate as project_domain;
use crate::{
    apply_geometry_snapshot_recursive, build_page_root, clamp_to_parent_bounds, page_size,
    set_element_geometry, transform_descendant_geometry_with_parent,
};
use core_blueprint::{
    blueprint_name_for_page, compile_project, validate_project, BlueprintCompilationResult,
    BlueprintDiagnostic, BlueprintDocument, BlueprintDocumentKind, BlueprintFunctionParameter,
    BlueprintFunctionSignature, BlueprintGraph, BlueprintGraphKind, BlueprintLink,
    BlueprintLocalVariable, BlueprintNode, BlueprintNodeKind, BlueprintOwner,
    BlueprintPinDirection, BlueprintPinKind, BlueprintPinType, BlueprintPoint, BlueprintProjectApi,
...
fn parse_input_pin_value(pin_type: BlueprintPinType, raw_value: &str) -> Option<serde_json::Value> {
    let trimmed = raw_value.trim();
    match pin_type {
        BlueprintPinType::Bool => match trimmed.to_ascii_lowercase().as_str() {
            "true" => Some(serde_json::json!(true)),
            "false" => Some(serde_json::json!(false)),
            _ => None,
        },
        BlueprintPinType::Int => trimmed
            .parse::<i64>()
            .ok()
            .map(|value| serde_json::json!(value)),
        BlueprintPinType::Float => trimmed
            .parse::<f64>()
            .ok()
            .filter(|value| value.is_finite())
            .map(|value| serde_json::json!(value)),
        BlueprintPinType::String => Some(serde_json::json!(raw_value)),
        BlueprintPinType::Color => (!trimmed.is_empty()).then(|| serde_json::json!(trimmed)),
...
pub use crate::CanvasElementData;
...
fn event_descriptor_ids_for_element_type(element_type: &str) -> Vec<&'static str> {
    match element_type {
        "button" => vec!["event.button"],
        "input" | "textarea" | "select" | "slider" => vec!["event.input"],
        "checkbox" | "radio" | "switch" => vec!["event.checkbox"],
        "label" => vec!["event.label"],
        "text" | "div" | "flex-container" | "grid-container" | "stack-container" | "image" => {
            vec!["event.fill"]
        }
        _ => Vec::new(),
    }
...
impl Project {
    ...
    pub fn page_id_by_index(&self, index: usize) -> Option<Uuid> {
        self.project_file
            .ui_data
            .pages
            .get(index)
    ...
    pub fn active_blueprint_diagnostics(&self) -> Vec<BlueprintDiagnostic> {
        let Some(active_document_id) = self.active_blueprint_document().map(|document| document.id)
        else {
            return Vec::new();
        };

        let mut documents = self.project_file.logic_data.documents.clone();
        for document in &mut documents {
            document.sync_exports();
        }

        let api = self.build_blueprint_api();
        validate_project(&documents, &api)
            .into_iter()
            .filter(|diagnostic| diagnostic.document_id == Some(active_document_id))
            .collect()
    }
    ...
    pub fn add_catalog_node_and_connect_from_active_blueprint(
        &mut self,
        from_node_id: Uuid,
        source_pin_kind: &str,
        source_pin_slot: i32,
        ...
        let document_kind = self.active_blueprint_document()?.kind;
        let graph = self.active_blueprint_graph_mut()?;
        let source_pin_kind = project_domain::SourcePinKind::from_str(source_pin_kind)?;
        let node_id = project_domain::add_catalog_node(
            graph,
            document_kind,
            descriptor_id,
            BlueprintPoint {
        ...
        .ok()
        .and_then(|outcome| outcome.affected_node_id)?;
        let drop_position = graph
            .nodes
            .iter()
            .find(|node| node.id == node_id)
            .map(|node| BlueprintPoint {
                x: node.position.x + 24,
                y: node.position.y + 24,
            })
            .unwrap_or(BlueprintPoint {
                x: preferred_x.round() as i32,
                y: preferred_y.round() as i32,
            });
        let _ = project_domain::connect_nodes_at(
            graph,
            from_node_id,
            source_pin_kind,
    ...
    pub fn set_input_pin_value_in_active_blueprint(
        &mut self,
        node_id: Uuid,
        input_slot: usize,
        raw_value: &str,
        ...
        let Some(next_value) = parse_input_pin_value(pin.data_type, raw_value) else {
            return false;
        };
        if pin.value.as_ref() == Some(&next_value) {
            return false;
        }
        pin.value = Some(next_value);
        true
    }
    ...
    pub fn duplicate_local_variable_in_active_blueprint(
        &mut self,
        variable_id: Uuid,
    ) -> Option<Uuid> {
        let graph = self.active_blueprint_graph_mut()?;
        let source = graph
            ...
            .find(|variable| variable.id == variable_id)
            .cloned()?;
        let mut name = format!("{}_copy", source.name);
        let mut copy_index = 2usize;
        while graph
            .local_variables
            .iter()
            .any(|variable| variable.name == name)
        {
            name = format!("{}_copy{}", source.name, copy_index);
            copy_index += 1;
        }

        let duplicated = BlueprintLocalVariable {
            id: Uuid::new_v4(),
            name,
        ...
        graph.local_variables.push(duplicated);
        Some(duplicated_id)
    }

    pub fn delete_local_variable_from_active_blueprint(&mut self, variable_id: Uuid) -> bool {
        let Some(graph) = self.active_blueprint_graph_mut() else {
            return false;
        };
        if !graph
        ...
        let removed_node_ids: HashSet<Uuid> = graph
            ...
            .filter_map(|node| match node.kind {
                ...
                | BlueprintNodeKind::VariableSet {
                    variable_id: node_variable_id,
                } if node_variable_id == variable_id => Some(node.id),
                _ => None,
            })
            .collect();
        ...
        for node in &mut graph.nodes {
            match node.kind {
                BlueprintNodeKind::VariableGet {
                    ...
                    sync_variable_getter_node(node, &variable);
                }
                BlueprintNodeKind::VariableSet {
                    variable_id: node_variable_id,
                } if node_variable_id == variable_id => {
                    sync_variable_setter_node(node, &variable);
                }
                _ => {}
            }
        }

        let _ = project_domain::prune_incompatible_links(graph);
    ...
    pub fn ensure_element_event_nodes_on_active_page_blueprint(
        ...
        let descriptor_ids = event_descriptor_ids_for_element_type(&element.element_type);
        ...
        for (idx, descriptor_id) in descriptor_ids.iter().enumerate() {
            let exists = graph.nodes.iter().any(|node| {
                matches!(
                    &node.kind,
                    core_blueprint::BlueprintNodeKind::CatalogEvent {
                        descriptor_id: node_descriptor_id,
                        element_id: node_element_id,
                    } if *node_element_id == element_id && node_descriptor_id == descriptor_id
                )
            });
            if exists {
                continue;
            ...
            let Some(node_id) = project_domain::add_catalog_node(
                graph,
            ...
            .ok()
            .and_then(|outcome| outcome.affected_node_id) else {
                continue;
            };
            if project_domain::bind_catalog_event_node_to_element_type(
                graph,
                BlueprintDocumentKind::PageBlueprint,
                node_id,
                element_id,
            ...
            .is_err()
            {
                let _ = project_domain::delete_node(graph, node_id);
                continue;
            }
            changed = true;
        }
...
fn blueprint_pin_type_is_collection(data_type: BlueprintPinType) -> bool {
    data_type.is_collection()
}

fn sync_variable_getter_node(node: &mut BlueprintNode, variable: &BlueprintLocalVariable) {
    node.title = variable.name.clone();
    if let Some(pin) = node.pins.iter_mut().find(|pin| pin.name == "value") {
        pin.data_type = variable.data_type;
    } else {
        node.pins.push(core_blueprint::BlueprintPin::data_output(
            "value",
            variable.data_type,
        ));
    }
...
fn sync_variable_setter_node(node: &mut BlueprintNode, variable: &BlueprintLocalVariable) {
    node.title = format!("Set {}", variable.name);
    ...
    next_pins.push(
        take_pin(
            ...
            BlueprintPinKind::Exec,
        )
        .unwrap_or_else(|| core_blueprint::BlueprintPin::exec_input("in")),
    );

    let value_type = if variable.data_type.is_collection() {
        variable.item_type.unwrap_or(BlueprintPinType::String)
    } else {
        variable.data_type
    };
    let mut value_pin = take_pin(
        &mut node.pins,
        "value",
        BlueprintPinDirection::Input,
        BlueprintPinKind::Data,
    )
    .unwrap_or_else(|| core_blueprint::BlueprintPin::data_input("value", value_type));
    if value_pin.data_type != value_type {
        value_pin.data_type = value_type;
        value_pin.value = core_blueprint::default_input_pin_value(value_type);
    }
    next_pins.push(value_pin);

    if variable.data_type.is_collection() {
        let mut mode_pin = take_pin(
            &mut node.pins,
            "mode",
            BlueprintPinDirection::Input,
            BlueprintPinKind::Data,
        ...
        .unwrap_or_else(|| {
            let mut pin =
                core_blueprint::BlueprintPin::data_input("mode", BlueprintPinType::String);
            pin.value = Some(serde_json::json!("push"));
            pin
        });
        mode_pin.data_type = BlueprintPinType::String;
        if mode_pin
            .value
            .as_ref()
            .and_then(|value| value.as_str())
    ...
    next_pins.push(
        ...
        .unwrap_or_else(|| core_blueprint::BlueprintPin::exec_output("then")),
    );
    node.pins = next_pins;
}

fn take_pin(
    pins: &mut Vec<core_blueprint::BlueprintPin>,
    name: &str,
    direction: BlueprintPinDirection,
    kind: BlueprintPinKind,
) -> Option<core_blueprint::BlueprintPin> {
    let index = pins
        .iter()
        .position(|pin| pin.name == name && pin.direction == direction && pin.kind == kind)?;
    Some(pins.remove(index))
}
...
mod tests {
    ...
    fn deleting_scene_element_removes_bound_blueprint_nodes() {
        ...
        assert!(project
            ...
            .all(|node| !matches!(
                node.kind,
                BlueprintNodeKind::CatalogEvent { element_id, .. } if element_id == button_id
            )));
    }
    ...
    #[test]
    fn project_updates_unconnected_blueprint_input_pin_values() {
        let temp = tempdir().expect("tempdir");
        let project_dir = temp.path().to_string_lossy().to_string();
        let mut project = Project::new(
            "SnappixInputPinValues",
            &project_dir,
            Platform::Desktop,
            DevMode::Nodes,
            PageSize::Desktop,
        ...
        let branch_id = project
            .add_catalog_node_to_active_blueprint("flow.branch", 80.0, 80.0)
            .expect("branch");
        assert!(project.set_input_pin_value_in_active_blueprint(branch_id, 0, "false"));

        let add_id = project
            .add_catalog_node_to_active_blueprint("math.int.add", 160.0, 160.0)
            .expect("int add");
        assert!(project.set_input_pin_value_in_active_blueprint(add_id, 0, "41"));
        assert!(project.set_input_pin_value_in_active_blueprint(add_id, 1, "1"));
        assert!(!project.set_input_pin_value_in_active_blueprint(add_id, 0, "41.5"));

        let document = project.active_blueprint_document().expect("blueprint");
        let branch = document.graphs[0]
            .node_by_id(branch_id)
            .expect("branch node");
        assert_eq!(
            branch
                .pin_named("condition")
        ...
        let add = document.graphs[0].node_by_id(add_id).expect("add node");
        let add_inputs: Vec<_> = add
            .pins
            .iter()
            .filter(|pin| {
            ...
            .collect();
        assert_eq!(
            add_inputs.first().and_then(|pin| pin.value.as_ref()),
            Some(&serde_json::json!(41))
        );
        assert_eq!(
            add_inputs.get(1).and_then(|pin| pin.value.as_ref()),
    ...
    #[test]
    fn deleting_variable_removes_variable_nodes_and_links() {
        let temp = tempdir().expect("tempdir");
        let project_dir = temp.path().to_string_lossy().to_string();
        let mut project = Project::new(
            "SnappixVariables",
        ...
        let getter_id = project
            ...
            .expect("getter");
        let setter_id = project
            .add_variable_node_to_active_blueprint(variable_id, "set", 260.0, 0.0)
            .expect("setter");

        {
            let graph = project.active_blueprint_graph_mut().expect("graph");
            let from_pin = graph
                .node_by_id(getter_id)
                .and_then(|node| node.pin_named("value"))
                .map(|pin| pin.id)
                .expect("getter pin");
            let to_pin = graph
                .node_by_id(setter_id)
                .and_then(|node| node.pin_named("value"))
                .map(|pin| pin.id)
                .expect("setter pin");
            graph
                .links
                .push(BlueprintLink::new(getter_id, from_pin, setter_id, to_pin));
        }

        assert!(project.delete_local_variable_from_active_blueprint(variable_id));
        let graph = project
            .active_blueprint_document()
            .and_then(|document| document.graphs.first())
            .expect("graph");
        assert!(graph.local_variables.is_empty());
        assert!(graph.nodes.iter().all(|node| {
            !matches!(
                node.kind,
                BlueprintNodeKind::VariableGet { .. } | BlueprintNodeKind::VariableSet { .. }
            )
        }));
    ...
    #[test]
    fn array_variable_setter_gets_value_and_mode_pins() {
        let temp = tempdir().expect("tempdir");
        let project_dir = temp.path().to_string_lossy().to_string();
        let mut project = Project::new(
            "SnappixArrays",
        ...
        let variable_id = project
            .add_local_variable_to_active_blueprint()
            .expect("variable");
        assert!(project.set_local_variable_type_in_active_blueprint(variable_id, "Array"));
        assert!(project.set_local_variable_item_type_in_active_blueprint(variable_id, "Object"));
        let setter_id = project
            .add_variable_node_to_active_blueprint(variable_id, "set", 0.0, 0.0)
            .expect("setter");
        ...
        let setter = project
            .active_blueprint_document()
            .and_then(|document| document.graphs.first())
            .and_then(|graph| graph.node_by_id(setter_id))
            .expect("setter node");
        let value_pin = setter.pin_named("value").expect("value pin");
        let mode_pin = setter.pin_named("mode").expect("mode pin");
        assert_eq!(value_pin.data_type, BlueprintPinType::Object);
        assert_eq!(mode_pin.data_type, BlueprintPinType::String);
        assert_eq!(
            mode_pin.value.as_ref().and_then(|value| value.as_str()),
            Some("push")
        );
    }

    #[test]
    fn creating_node_from_wire_connects_to_compatible_input() {
        let temp = tempdir().expect("tempdir");
        let project_dir = temp.path().to_string_lossy().to_string();
        let mut project = Project::new(
            "SnappixCreateConnect",
            &project_dir,
            Platform::Desktop,
            DevMode::Nodes,
            PageSize::Desktop,
        ...
        let source_id = project
            .add_catalog_node_to_active_blueprint("value.bool_true", 0.0, 0.0)
            .expect("source node");
        let branch_id = project
        ...
        assert!(graph
            .links
            .iter()
            .any(|link| { link.from_node_id == source_id && link.to_node_id == branch_id }));
    }

    #[test]
    fn active_blueprint_diagnostics_reports_node_validation_errors() {
        let temp = tempdir().expect("tempdir");
        let project_dir = temp.path().to_string_lossy().to_string();
        let mut project = Project::new(
            "SnappixLiveDiagnostics",
            &project_dir,
            Platform::Desktop,
            DevMode::Nodes,
            PageSize::Desktop,
        ...
        let array_variable_id = project
            .add_local_variable_to_active_blueprint()
            .expect("array variable");
        assert!(project.set_local_variable_type_in_active_blueprint(
            array_variable_id,
            "Array"
        ));
        assert!(project.set_local_variable_item_type_in_active_blueprint(
            array_variable_id,
            "Object"
        ));
        let setter_id = project
            .add_variable_node_to_active_blueprint(array_variable_id, "set", 260.0, 0.0)
            .expect("setter");

        let string_variable_id = project
            .add_local_variable_to_active_blueprint()
        ...
        let getter_id = project
            .add_variable_node_to_active_blueprint(string_variable_id, "get", 0.0, 0.0)
            .expect("getter");

        {
            ...
            let to_pin = graph
                .node_by_id(setter_id)
                .and_then(|node| node.pin_named("mode"))
                .map(|pin| pin.id)
                .expect("setter mode");
            graph
                .links
        ...
        let diagnostics = project.active_blueprint_diagnostics();
        assert!(diagnostics.iter().any(|diagnostic| {
            diagnostic.node_id == Some(setter_id) && diagnostic.code == "collection_mode_empty"
        }));
    }
}
