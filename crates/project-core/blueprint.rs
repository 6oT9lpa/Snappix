...
pub fn connect_nodes_at(
    ...
    graph
        .links
        .retain(|link| link.from_pin_id != source_pin_id && link.to_pin_id != target_pin_id);
...
pub fn blueprint_node_visual_size(node: &BlueprintNode) -> (i32, i32) {
    ...
    let height = if is_variable_getter {
        60
    } else if exec_output_count >= 3 {
        150
    } else if exec_output_count >= 2 {
        126
    } else if has_data_input {
        118
    } else {
        94
    };
    let width = if is_event { 250 } else { 270 };
    (width, height)
}
...
mod tests {
    use super::*;
    use core_blueprint::{BlueprintGraphKind, BlueprintPin};
